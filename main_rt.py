...
# LinearRegression creates the pipeline that fitr the model that we're
# interested in. Here, the model is a linear one, where the coefficientr of the
...
pointr = np.loadtxt('data/training_params_rt.txt')
...
def get_cl(tau, r, consider='EE', degree=5):
    ...
    predict = np.array([r, tau]).reshape(-1,2)
    # Creates matrix of values you want to estimate from the existing
    # measurementr. Computation speed scales very slowly when you ask for
    # estimate of many setr of parameters.
...
if __name__ == '__main__':
    ...
    plt.savefig('plots/tau_example_rt.png')
    ...
    plt.savefig('plots/s_example_rt.png')
    plt.show()
