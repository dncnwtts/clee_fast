...
def C_l(s, tau, ps='BB'):
    ...
    pars.InitPower.set_params(ns=1.0, r=0, As=2.3e-9*s)
...
if new == True:
    try:
        ...
        values_EE = np.loadtxt('training_data_EE_ts.txt')
        values_BB = np.loadtxt('training_data_BB_ts.txt')
    except IOError:
    ...
    for i in range(ntrain):
        ...
        if i % 10 == 0:
            print(i, new[i])
    print('\n\n\n\n')
    np.savetxt('../data/training_data_EE_ts.txt', values_EE)
    np.savetxt('../data/training_data_BB_ts.txt', values_BB)
    np.savetxt('../data/training_params_ts.txt', points)
else:
    values_EE = np.loadtxt('../data/training_data_EE_ts.txt')
    values_BB = np.loadtxt('../data/training_data_BB_ts.txt')
    points = np.loadtxt('../data/training_params_ts.txt')
if consider == 'EE':
    values = values_EE
else:
    values = values_BB
...
for i, p in enumerate(predict[:50]):
    ...
    plt.plot(x, label=r'$s={0}, \tau={1}$'.format(*p))
...
if show:
    ...
    import corner
    corner.corner(points, labels=[r'$s$', r'$\tau$'])
    plt.savefig('../plots/point_density_ts')
    ...
    plt.title(r'$C_{{\ell=10}}^\mathrm{{ {0} }}(\tau,A_s)$ (fit with degree-{1} polynomial)'.format(consider, degree), size=20)

    plt.savefig('../plots/single_ell_accuracy_ts.png')
    plt.show()
